...
{ //calibrated camera
    ...
    bool operator()(
                    const T *const camera_pose,
                    const T *const point_x,         //world coordinate
                    const T *const point_y,
                    const T *const point_z,
                    const T *const camera_intrin,//fx,fy,cx,cy
                    T *residuals
                    ) const
    {
        ...
        point[1] = *point_y;
        point[2] = *point_z;
        // std::cout<<"real xyz: "<<point[0]<<", "<<point[1]<<", "<<point[2];
        ceres::AngleAxisRotatePoint(camera_pose, point, p);
        // camera_pose[3,4,5] are the translation.
        p[0] += camera_pose[3];
        p[1] += camera_pose[4];
    ...
    static ceres::CostFunction *Create(const double observed_x,
                                       const double observed_y)
    {
        return (new ceres::AutoDiffCostFunction<TReprojectionError, 2, 6, 1,1,1, 4>(
            new TReprojectionError(observed_x, observed_y)));
    }
